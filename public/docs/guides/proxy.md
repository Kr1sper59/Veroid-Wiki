...
> ⚠️ Лучше всего покупать сервер для прокси на одном узле(ноде) с серверами, которые вы хотите связать с помощью него.
>
> ⚠️ При подключении серверов на одной ноде лучше всего использовать локальный Ip адрес: 127.0.0.1
## Velocity
...
### congig.yml
Основная настройка выполняется в файле **config.yml**.
...
servers:
  lobby:
    ...
    address: 127.0.0.1:10002  # Укажите адрес вашего лобби-сервера
    restricted: false
  main:
    motd: '&1Just another Server'
    address: 127.0.0.1:10001  # Укажите адрес вашего основного сервера
    restricted: false
disabled_commands:
- disabledcommandhere
...
### locations.yml
Этот файл хранит список **сохранённых локаций** (точек телепортации) для команды **/server**. Обычно заполняется автоматически плагинами и вручную редактировать его не нужно.

### modules.yml

Этот файл управляет **модулями Waterfall** - встроенными командами и функциями прокси. По умолчанию подключены:

| Модуль | Описание |
|---|---|
| **cmd_alert** | Команда **/alert** - отправка сообщений всем игрокам |
| **cmd_find** | Команда **/find** - поиск игрока по нику |
| **cmd_list** | Команда **/list** - список игроков онлайн |
| **cmd_send** | Команда **/send** - перемещение игрока на другой сервер |
| **cmd_server** | Команда **/server** - смена сервера |
| **reconnect_yaml** | Возвращает игрока на последний сервер после реконнекта |

Редактировать этот файл нужно только если хотите отключить какую-то из встроенных команд.

### waterfall.yml
Файл с **дополнительными настройками Waterfall**, которых нет в стандартном BungeeCord. Большинство параметров можно оставить по умолчанию, но стоит знать о ключевых:

| Параметр | Описание |
|---|---|
| **game_version** | Принудительная версия клиента. Оставьте пустым для поддержки всех версий |
| **disable_modern_tab_limiter** | Снимает лимит на количество игроков в табе |
| **disable_tab_list_rewrite** | Отключает перезапись таб-листа прокси |
| **throttling**tab_complete` | Задержка в мс между запросами автодополнения команд |
| **use_netty_dns_resolver** | Использует Netty для DNS-резолвинга (рекомендуется оставить **true**) |

> ⚠️ Эти файлы трогать не обязательно - они уже настроены оптимально для большинства серверов.